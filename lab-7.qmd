...
create_passtimes_df <- function(df, state_col, lat_col, long_col, minelevation = 70, hours = 12) {
  ...
  for (i in 1:nrow(return_df)) {
    ...
    return_df[i, c(4, 5, 6)] <- cur_pass_times[, c(1, 2, 3)]
    Sys.sleep(1) # get sys sleep so api dont get angry
  }
...
```{r}
# Try it
a <- create_passtimes_df(state_ll, state, lat, long, minelevation = 30, hours = 48)
a
```

```{r}
final_dataset <- create_passtimes_df(state_ll, state, lat, long, minelevation = 30, hours = 48)
final_dataset <- final_dataset |> 
  mutate(across(time1:time3, ~as_datetime(.x))) |> 
  arrange(time1) |> 
...
final_dataset |> 
  ...
  addMarkers(
    ...
    popup = paste0("State Capitol Name: ", " ", final_dataset$name, " <br> ",
                   ...
                   "Third Time: <br> ",
                   final_dataset$time3),
    label = paste0("State Capitol Name: ", " ", final_dataset$name, " <br> ",
